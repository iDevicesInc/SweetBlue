	...
	void disconnectAllForTurnOff(PE_TaskPriority priority)
	{
		synchronized (m_list)
		{
			for( int i = m_list.size()-1; i >= 0; i-- )
			{
				...
				{
					device.disconnectWithReason(priority, Reason.BLE_TURNING_OFF, ConnectionFailListener.Timing.NOT_APPLICABLE, BleDeviceConfig.GATT_STATUS_NOT_APPLICABLE, BleDeviceConfig.BOND_FAIL_REASON_NOT_APPLICABLE);
				}
			}
		}
	}
