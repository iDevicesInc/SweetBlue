...
//			}
        ...
        if (e.failureCountSoFar() <= m_retryCount)
        {
            ...
            {
                if (e.status() == DeviceConnectionFailListener.Status.NATIVE_CONNECTION_FAILED && e.timing() == DeviceConnectionFailListener.Timing.TIMED_OUT)
                {
                    if (e.autoConnectUsage() == NodeConnectionFailListener.AutoConnectUsage.USED)
                    {
                        return Please.retryWithAutoConnectFalse();
                    }
                    else if (e.autoConnectUsage() == NodeConnectionFailListener.AutoConnectUsage.NOT_USED)
                    {
                        return Please.retryWithAutoConnectTrue();
                    }
                    else
