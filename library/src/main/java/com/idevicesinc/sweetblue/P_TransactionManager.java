	...
	final PI_EndListener m_txnEndListener = new PI_EndListener()
	{
		@Override public void onTransactionEnd(BleTransaction txn, EndReason reason, ReadWriteListener.ReadWriteEvent txnFailReason)
		{
			...
			if (txn == m_authTxn )
			{
				...
				{
					m_device.disconnectWithReason(Status.AUTHENTICATION_FAILED, DeviceReconnectFilter.Timing.NOT_APPLICABLE, BleStatuses.GATT_STATUS_NOT_APPLICABLE, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, txnFailReason);
				}
			}
			else if (txn == m_initTxn )
			{
				...
				{
					m_device.disconnectWithReason(Status.INITIALIZATION_FAILED, DeviceReconnectFilter.Timing.NOT_APPLICABLE, BleStatuses.GATT_STATUS_NOT_APPLICABLE, BleStatuses.BOND_FAIL_REASON_NOT_APPLICABLE, txnFailReason);
				}
			}
